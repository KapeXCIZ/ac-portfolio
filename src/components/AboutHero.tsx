...
export default function AboutHero() {
    ...
    return (
        <div className={cn("relative h-[75vh] max-h-[650px]  md:max-h-[800px] -z-40   overflow-y-hidden w-full flex flex-col justify-end items-center max-w-screen overflow-x-hidden transition duration-300 ease-in-out", !imageVisible ? "opacity-0" : "opacity-100")} >
            <div className="fixed h-[75vh] max-h-[650px]  md:max-h-[800px] top-20 w-full flex flex-col justify-start  items-center  blur-(--about-hero-blur) -z-10">

                <div className={cn(" flex flex-col absolute top-0  w-full h-full text-accent-700 dark:text-accent-200    *:tracking-[.125em] *:md:tracking-[.175em]  *:xl:tracking-[.25em]  -z-50 ", collapsed ? "*:m-0! *:tracking-[0em] *:md:tracking-[0em] *:xl:tracking-[0em] " : "")}>
                    {elements}
                </div>
                <div className={cn("relative isolate w-full h-full  opacity-100 transition-opacity duration-300 ", !imageVisible ? "opacity-0" : "")}>
                    <Image src={"/images/about/1.png"} width={1000} height={1500} alt="Thoughtful Alessio" className={cn("h-[600px] max-h-[70vh] md:h-[75vh]  px-4 absolute  left-1/2 w-auto  -translate-x-1/2 bottom-0    saturate-85 dark:saturate-75  duration-500 ease-in  md:max-h-[800px]  brightness-115 dark:brightness-100   peer opacity-100 -z-1 contrast-90 dark:contrast-110", collapsed ? "opacity-0" : "")} />
